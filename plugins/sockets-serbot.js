...
await creloadHandler(false) 
  ...
  console.error('❌ creloadHandler falló:', e)
}

if (!reloadOk) {
  // intentar reparar handler
  await creloadHandler(false)
} 
}
if (reason === 403) {
...
function isZombie(sock) {
  // no existe sesión
  if (!sock) return false

  // no autenticado → no es zombi
